...
async function getExhibitions(): Promise<Exhibition[]> {
  const result = await safeSanityFetch<Exhibition[]>(EXHIBITIONS_COMBINED_QUERY, { language: "it" }, { next: { revalidate: 60 } })
  return result || []
}
...
export default async function MostrePage() {
  const [exhibitions, settings] = await Promise.all([getExhibitions(), getSettings()])
